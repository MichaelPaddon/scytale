...
#[inline(always)]
fn sub_word(x: Word) -> Word {
    let mut bytes = Word::to_ne_bytes(x);
    for i in 0..mem::size_of::<Word>() {
    ...
    for c in 0..NB {
        (s[c][0], s[c][1], s[c][2], s[c][3]) = (
            GF_MUL_2[s[c][0] as usize] ^ GF_MUL_3[s[c][1] as usize]
                ^ s[c][2] ^ s[c][3],
            s[c][0] ^ GF_MUL_2[s[c][1] as usize]
                ^ GF_MUL_3[s[c][2] as usize] ^ s[c][3],
            s[c][0] ^ s[c][1]
                ^ GF_MUL_2[s[c][2] as usize] ^ GF_MUL_3[s[c][3] as usize],
            GF_MUL_3[s[c][0] as usize] ^ s[c][1]
                ^ s[c][2] ^ GF_MUL_2[s[c][3] as usize]
        );
    }
}
    ...
    for c in 0..NB {
        (s[c][0], s[c][1], s[c][2], s[c][3]) = (
            GF_MUL_14[s[c][0] as usize] ^ GF_MUL_11[s[c][1] as usize]
                ^ GF_MUL_13[s[c][2] as usize] ^ GF_MUL_9[s[c][3] as usize],
            GF_MUL_9[s[c][0] as usize] ^ GF_MUL_14[s[c][1] as usize]
                ^ GF_MUL_11[s[c][2] as usize] ^ GF_MUL_13[s[c][3] as usize],
            GF_MUL_13[s[c][0] as usize] ^ GF_MUL_9[s[c][1] as usize]
                ^ GF_MUL_14[s[c][2] as usize] ^ GF_MUL_11[s[c][3] as usize],
            GF_MUL_11[s[c][0] as usize] ^ GF_MUL_13[s[c][1] as usize]
                ^ GF_MUL_9[s[c][2] as usize] ^ GF_MUL_14[s[c][3] as usize]
        );
    }
}
