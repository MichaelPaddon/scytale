...
pub fn mwp128() {
    ...
    let ciphertext = [0x39, 0x25, 0x84, 0x1d, 0x02, 0xdc, 0x09, 0xfb, 0xdc, 0x11, 0x85, 0x97, 0x19, 0x6a, 0xb, 0x32];

    let aes = Aes128::new(&key).unwrap();
    let mut output = [0u8; 16];
    aes.encrypt(plaintext.as_slice(), output.as_mut_slice());
    assert_eq!(output, ciphertext);
    aes.decrypt(ciphertext.as_slice(), output.as_mut_slice());
    assert_eq!(output, plaintext);

    /*
    let plaintext = [0u8; 65536];
