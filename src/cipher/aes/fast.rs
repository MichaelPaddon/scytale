...
{
    w: Array<Word, op!(NB * (NR + U1))>,
    dw: Option<Array<Word, op!(NB * (NR + U1))>>,
    _nk: PhantomData<NK>
}

impl<NB: ArraySize, NK: ArraySize, NR: ArraySize> Aes<NB, NK, NR>
where
    NR: Add<U1>,
    NB: Mul<Sum<NR, U1>>,
    ...
    Diff<Prod<NB, Sum<NR, U1>>, U1>: ArraySize,
{
        ...
        -> Result<Array<Word, op!(NB * (NR + U1))>, Error>
    {
        ...
        for (i, n) in iter::zip(
            ...
            if n == 0 {
                t = TE2[(t >> 16 & 0xff) as usize] & 0xff000000
                    ^ TE3[(t >> 8 & 0xff) as usize] & 0x00ff0000
                    ^ TE0[(t & 0xff) as usize] & 0x0000ff00
                    ^ TE1[(t >> 24) as usize] & 0x000000ff
                    ^ RCON[rcon];
                rcon += 1;
            }
            else if NK::USIZE > 6 && n == size_of::<Word>() {
                t = TE2[(t >> 24) as usize] & 0xff000000
                    ^ TE3[(t >> 16 & 0xff) as usize] & 0x00ff0000
                    ^ TE0[(t >> 8 & 0xff) as usize] & 0x0000ff00
                    ^ TE1[(t & 0xff) as usize] & 0x000000ff;
            }

            t ^= unsafe {
                w[i - NK::USIZE].assume_init_read()
            };
    ...
    fn invert_key(w: &Array<Word, op!(NB * (NR + U1))>)
        -> Array<Word, op!(NB * (NR + U1))>
    {
        let mut dw: Array<MaybeUninit<Word>, op!(NB * (NR + U1))> =
            Array::uninit();

        let inv_mix_columns = |x: Word| {
            TD0[(TE1[(x >> 24) as usize] & 0xff) as usize]
            ^ TD1[(TE1[(x >> 16 & 0xff) as usize] & 0xff) as usize]
            ^ TD2[(TE1[(x >> 8 & 0xff) as usize] & 0xff) as usize]
...
impl<NB: ArraySize, NK: ArraySize, NR: ArraySize> Rekey for  Aes<NB, NK, NR>
where
    NR: Add<U1>,
    NB: Mul<Sum<NR, U1>>,
    Prod<NB, Sum<NR, U1>>: ArraySize + Sub<U1>,
...
impl<NB: ArraySize, NK: ArraySize, NR: ArraySize> EncryptBlocks
    for  Aes<NB, NK, NR>
where
    NR: Add<U1>,
    NB: Mul<Sum<NR, U1>>,
    ...
    Prod<NB, WORD_SIZE>: ArraySize,
{
    fn encrypt_blocks(
        ...
        for (pt, ct) in iter::zip(plaintext, ciphertext) {
            let mut state: Array<MaybeUninit<Word>, NB> = Array::uninit();
            let p: *const Word = pt.as_ptr().cast();
            for i in 0..NB::USIZE {
            ...
            for r in 1..NR::USIZE {
                ...
                for j in 0..NB::USIZE {
                    t[j].write(self.w[i]
                        ^ TE0[(state[j] >> 24 & 0xff) as usize]
                        ^ TE1[(state[(j + 1) % NB::USIZE] >> 16 & 0xff) as usize]
                        ^ TE2[(state[(j + 2) % NB::USIZE] >> 8 & 0xff) as usize]
                        ^ TE3[(state[(j + 3) % NB::USIZE] & 0xff) as usize]
                    );
                    i += 1;
                }
                state = unsafe {
            ...
            for j in 0..NB::USIZE {
                let x =  (SBOX[(state[j] >> 24 & 0xff) as usize] as Word) << 24
                    | (SBOX[(state[(j + 1) % NB::USIZE] >> 16 & 0xff) as usize] as Word) << 16
            ...
            */
            ...
            let p: *mut Word = ct.as_mut_ptr().cast();
            for i in 0..NB::USIZE {
                unsafe {
...
fn mwp(){
    ...
    let mut ct2: [Array<u8, U16>; 1] = [Array([0u8; 16])];
    ...
    let a = super::soft::Aes128::new(&key).unwrap();
    assert_eq!(x, a.w);

    let key = [0u8; 24];
    let x = Aes::<U4, U6, U12>::expand_key(&key).unwrap();
    let a = super::soft::Aes192::new(&key).unwrap();
    assert_eq!(x, a.w);
