            ...
            let key_bits = untrusted::Input::from(&public_key)
                .read_all(error::Unspecified, |input| {
                    der::nested(
                        ...
                        error::Unspecified,
                        |input| {
                            let n_bytes = der::positive_integer(input)?
                            ...
                            let n_leading_zeros =
                                usize::try_from(n_bytes[0].leading_zeros())
                                    .unwrap();
...
fn test_public_key_coverage(key: &rsa::PublicKey) {
    ...
    const PUBLIC_KEY_DEBUG: &str =
        include_str!("rsa_test_public_key_2048_debug.txt");
    assert_eq!(PUBLIC_KEY_DEBUG, format!("{:?}", key));

    let components = rsa::PublicKeyComponents::<Vec<_>>::from(key);
    const PUBLIC_KEY_MODULUS_BE_BYTES: &[u8] =
        include_bytes!("rsa_test_public_modulus.bin");
