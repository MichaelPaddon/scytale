...
fn test_signature_ed25519_verify() {
    test::run(
        ...
        |section, test_case| {
            ...
            let expected_result =
                match test_case.consume_string("Result").as_str() {
                    "P" => Ok(()),
...
fn test_ed25519_from_seed_and_public_key_misuse() {
    ...
    assert!(
        Ed25519KeyPair::from_seed_and_public_key(PUBLIC_KEY, PRIVATE_KEY)
            .is_err()
    );
}
        ...
        |section, test_case| {
            ...
            let expected_error = {
                let expected_checked =
                    test_case.consume_string("Result-Checked");
                let expected_maybe_unchecked =
                    test_case.consume_string("Result-Maybe-Unchecked");
                let expected_result = match variant {
                    FromPkcs8Variant::Checked => expected_checked,
                    FromPkcs8Variant::MaybeUnchecked => {
                        expected_maybe_unchecked
                    }
                };
            ...
            let expected_public = {
                let expected_if_no_error =
                    test_case.consume_optional_bytes("Public");
...
fn ed25519_test_public_key_coverage() {
    ...
    const PUBLIC_KEY_DEBUG: &str = "PublicKey(\"5809e9fef6dcec58f0f2e3b0d67e9880a11957e083ace85835c3b6c8fbaf6b7d\")";
    ...
    let _: <Ed25519KeyPair as KeyPair>::PublicKey =
