...
use wasm_bindgen_test::{
    wasm_bindgen_test as test, wasm_bindgen_test_configure,
};
...
fn test_open_in_place_seperate_tag(
    ...
    {
        let nonce = aead::Nonce::assume_unique_for_key(tc.nonce);
        let actual_plaintext = key.open_in_place_separate_tag(
            nonce,
            tc.aad,
            tag,
            &mut in_out,
...
fn sealing_key_seal_in_place_append_tag(
    ...
    test_seal_append_tag(&tc, |nonce, in_out| {
        let mut key: aead::SealingKey<OneNonceSequence> =
            make_key(alg, tc.key, nonce);
...
fn key_sizes(aead_alg: &'static aead::Algorithm) {
    ...
    assert!(
        aead::UnboundKey::new(aead_alg, &key_data[..(key_len - 1)]).is_err()
    );
    ...
    assert!(
        aead::UnboundKey::new(aead_alg, &key_data[..(key_len / 2)]).is_err()
    );
...
fn test_aead_nonce_sizes() {
    ...
    assert!(
        aead::Nonce::try_assume_unique_for_key(&nonce[..nonce_len]).is_ok()
    );
    assert!(
        aead::Nonce::try_assume_unique_for_key(&nonce[..(nonce_len - 1)])
            .is_err()
    );
    assert!(
        aead::Nonce::try_assume_unique_for_key(&nonce[..(nonce_len + 1)])
    ...
    assert!(
        aead::Nonce::try_assume_unique_for_key(&nonce[..(nonce_len / 2)])
            .is_err()
    );
    assert!(
        aead::Nonce::try_assume_unique_for_key(&nonce[..(nonce_len * 2)])
    ...
    test::run(
        ...
        |section, test_case| {
            ...
            let key_bytes = {
                ...
                let mut as_array =
                    [0u8; aead::chacha20_poly1305_openssh::KEY_LEN];
            ...
            let s_key =
                aead::chacha20_poly1305_openssh::SealingKey::new(&key_bytes);
            s_key.seal_in_place(sequence_num, &mut s_in_out[..], &mut tag);
            assert_eq!(&ct, &s_in_out);
            assert_eq!(&expected_tag, &tag);
            let o_key =
                aead::chacha20_poly1305_openssh::OpeningKey::new(&key_bytes);

            {
                let o_result =
                    o_key.open_in_place(sequence_num, &mut s_in_out[..], &tag);
...
fn test_aead_key_debug() {
    ...
    let key: aead::LessSafeKey =
        make_less_safe_key(&aead::AES_256_GCM, &key_bytes);
    assert_eq!(
...
fn test_aead_lesssafekey_clone_for_algorithm(
    ...
    let key1: aead::LessSafeKey = aead::LessSafeKey::new(
        aead::UnboundKey::new(algorithm, key_bytes).unwrap(),
    );
    let key2 = key1.clone();
