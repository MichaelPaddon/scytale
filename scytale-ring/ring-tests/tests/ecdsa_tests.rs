    ...
    test::run(
        ...
        |section, test_case| {
            ...
            match (
                ...
                (Ok(_), Some(e)) => {
                    panic!("Succeeded, but expected error \"{}\"", e)
                }
            ...
            assert!(
                signature::EcdsaKeyPair::from_pkcs8(other_fixed, &input, &rng)
                    .is_err()
            );
            assert!(
                signature::EcdsaKeyPair::from_pkcs8(other_asn1, &input, &rng)
                    .is_err()
            );
...
fn ecdsa_generate_pkcs8_test() {
    ...
    for alg in &[
        ...
        #[cfg(feature = "alloc")]
        let _ = signature::EcdsaKeyPair::from_pkcs8(alg, pkcs8.as_ref(), &rng)
            .unwrap();
    }
}
    ...
    test::run(
        ...
        |section, test_case| {
            ...
            let alg = match (curve_name.as_str(), digest_name.as_str()) {
                ...
                _ => {
                    panic!(
                        "Unsupported curve+digest: {}+{}",
                        curve_name, digest_name
                    );
                }
            };
...
fn ecdsa_test_public_key_coverage() {
    ...
    const PUBLIC_KEY_DEBUG: &str =
        include_str!("ecdsa_test_public_key_p256_debug.txt");
    ...
    let _: <signature::EcdsaKeyPair as KeyPair>::PublicKey =
...
fn signature_ecdsa_sign_fixed_sign_and_verify_test() {
    ...
    test::run(
        ...
        |section, test_case| {
            ...
            let public_key =
                signature::UnparsedPublicKey::new(verification_alg, q);
