...
//! let cfb = Cfb1::<Aes128>::new(&Key::from([0u8; 16]));
//! let iv = [0u8; 16];
//!
...
impl<C: BlockCipher> fmt::Debug for Cfb1<C> {
...
mod tests {
    ...
    fn cfb<const K: usize>(key: &[u8; K]) -> Cfb1<Aes<K>> {
        Cfb1::new(&Key::from(*key))
    }
