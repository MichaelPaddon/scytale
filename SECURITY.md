...
| ECDH P-256, P-384 | `math/ec/`, `math/montgomery/` | complete projective formulas (Renes-Costello-Batina), so no special cases for doubling or the identity; a secret scalar in fixed 4-bit windows, or over four combs for the base point; every table read scans the whole table; inversion and square roots by exponentiation, whose exponent is a public constant and whose windows therefore may be slid over it, while the value being inverted steers nothing; on x86-64 with ADX the field product and square are the assembly in `math/montgomery/x86_64.rs`, at either width, a fixed instruction sequence whose final subtraction is chosen by a mask |
