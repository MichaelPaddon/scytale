...
| ECDH P-256, P-384 | `math/ec/`, `math/montgomery/` | the fixed-base multiplication uses the complete projective formulas (Renes-Costello-Batina), so no special cases for doubling or the identity; the variable-base one, which is where a secret scalar meets an untrusted point, runs in Jacobian coordinates over signed five-bit windows (Booth), where the addition is not complete and every case it misses is settled by a mask and not a branch: equal points take the double of the table entry, which is kept beside it, and either point being the identity takes the other; opposite points the formula handles itself. Every table read scans the whole table, and the digit only chooses and negates. Inversion and square roots are exponentiations whose exponent is a public constant, so their windows are slid over it while the value stays out of the timing; on x86-64 with ADX the field product and square are the assembly in `math/montgomery/x86_64.rs`, at either width, a fixed instruction sequence whose final subtraction is chosen by a mask |
