...
| Ed25519 verification | `sig/ed25519.rs::mul_add_base_vartime` | variable time, on public data only: one pass over both scalars in signed digits, with an addition for each nonzero digit, so which additions happen and from which entries is what the scalars are. See the table below |
| ECDH P-256, P-384 | `math/ec/`, `math/montgomery/` | the fixed-base multiplication reads per-window tables of affine multiples for P-256 (37 windows of 64 entries, signed seven-bit Booth digits, no doublings) and the complete projective formulas (Renes-Costello-Batina) over the combs for P-384, so neither has a special case for doubling or the identity; the variable-base one, which is where a secret scalar meets an untrusted point, runs in Jacobian coordinates over signed five-bit windows (Booth), where the addition is not complete and every case it misses is settled by a mask and not a branch: either point being the identity takes the other, opposite points the formula handles itself, and two equal points cannot arise there, which the module documents. Every table read scans the whole table, and the digit only chooses and negates; on x86-64 the 64-entry fixed-base scan is an AVX2 loop in `math/ec/x86_64.rs` that reads every entry whatever the digit, and elsewhere the compiler makes the same scan into 128-bit masked selects. Inversion and square roots are exponentiations whose exponent is a public constant, so their windows are slid over it while the value stays out of the timing; on x86-64 with ADX the field product and square are the assembly in `math/montgomery/x86_64.rs`, and on AArch64 the assembly in `math/montgomery/aarch64.rs`, at either width; and for P-256 the point doubling, both additions and the inversion chain are single assembly blocks, in `math/ec/x86_64.rs` and `math/ec/aarch64.rs`, each a fixed instruction sequence whose subtractions are chosen by mask
...
| ECDSA verification | `math/ec/mod.rs::mul_add_vartime` | variable time, on public data only: one pass over the public key's scalar in signed five-bit digits, with an addition only where a digit is nonzero, and the base point's multiple read from the static per-window table by an index the digit gives, both of which are what the scalars are. See the table below
...
| ECDSA verification (`mul_add_vartime`) | the signature, the public key and the message digest | all three are public: the caller was given the signature to check. The same caveat as Ed25519 below |
| Ed25519 verification (`mul_add_base_vartime`) | the signature, the public key and the message digest | all three are public: the caller was given the signature to check. Where a verifier holds a message an attacker does not, the timing says something about its digest, so do not verify a secret message where an attacker can time it |
